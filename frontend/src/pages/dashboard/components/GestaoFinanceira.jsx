...
export default function GestaoFinanceira() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const novaTransacao = await produtorService.registrarTransacao({
        tipo: tipoTransacao,
        ...formData,
  ...
  const categorias = {
    receita: [
      ...
      { value: 'venda_leite', label: 'Venda de Leite' },
      { value: 'venda_insumos', label: 'Venda de Insumos' },
        ...
        <CardContent>
          <div className="flex gap-4 mb-4">
            <Button
              ...
              className={tipoTransacao === 'receita' ? 'bg-green-600 hover:bg-green-700' : ''}
            >
            ...
            <Button
              ...
              className={tipoTransacao === 'despesa' ? 'bg-red-600 hover:bg-red-700' : ''}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button 
              type="submit" 
              className={tipoTransacao === 'receita' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}
              disabled={submitting}
            >
          ...
          {transacoes.length === 0 ? (
            ...
            <div className="space-y-3">
              {transacoes.slice(0, 10).map((transacao) => (
                <div key={transacao.id} className="flex justify-between items-center p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div>
                  ...
                  <div className={`font-bold ${
                    ...
                    {transacao.tipo === 'receita' ? '+' : '-'} R$ {transacao.valor?.toLocaleString() || 0}
                  </div>
                </div>
              ))}
            </div>
