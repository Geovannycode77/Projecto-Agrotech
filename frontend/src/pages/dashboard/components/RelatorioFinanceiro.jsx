      ...
      <div className="mt-6 p-4 bg-amber-50 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="text-center p-3 bg-white rounded-lg">
            ...
            <p className="text-2xl font-bold text-amber-600">{margemLucro.toFixed(1)}%</p>
            <p className="text-xs text-gray-400">Lucro / Receita total</p>
          </div>
          <div className="text-center p-3 bg-white rounded-lg">
            <p className="text-sm text-gray-600">Resultado do Período</p>
            <p className={`text-2xl font-bold ${relatorio.lucro >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
              {relatorio.lucro >= 0 ? 'Lucro' : 'Prejuízo'}
            </p>
            <p className="text-xs text-gray-400">AOA {Math.abs(relatorio.lucro).toLocaleString('pt-AO')}</p>
          </div>
        </div>
      </div>
      </CardContent>
